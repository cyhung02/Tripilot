            ...
            {days.map(day => (
              <button
                ...
                className={`w-full text-left py-3 px-4 hover:bg-opacity-50 transition ${
                  selectedDay === day.id ? `${themeColors.accent} text-white` : 'hover:bg-gray-100'
                }`}
              >
                {day.date} | {day.title}
              </button>
        ...
        >
          {days.map(day => (
            <button
              ...
              className={`whitespace-nowrap mx-1 py-2 px-4 rounded-full transition-all duration-300 focus:outline-none ${
                selectedDay === day.id
